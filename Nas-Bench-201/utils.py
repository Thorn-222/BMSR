...
def load_graph_config(graph_data_name, nvt, data_path):
  ...
  g_list = []
  max_n = 0
  ms = torch.load(os.path.join(data_path, f'{graph_data_name}.pt'))['arch']['matrix']
  for i in range(len(ms)):
    g, n = decode_NAS_BENCH_201_8_to_igraph(ms[i])
    max_n = max(max_n, n)
    g_list.append((g, 0))
  graph_config = {}
  graph_config['num_vertex_type'] = nvt
  graph_config['max_n'] = max_n
  graph_config['START_TYPE'] = 0
  graph_config['END_TYPE'] = 1
